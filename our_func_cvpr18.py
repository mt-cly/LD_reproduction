...
def cly_our_func(model_related, img, int_pos, int_neg, seq_points):
    sess, network, input, sz = model_related
    # adjust from PIL order to opencv order
    input_image = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
    iH, iW, _ = input_image.shape
    clk_x, clk_y, is_pos = seq_points[-1]

    clk_pos = (int_pos == 0)
    clk_neg = (int_neg == 0)
    if is_pos:
        clk_pos[clk_y, clk_x] = 1
        int_pos = ndimage.distance_transform_edt(1 - clk_pos)
        int_pos = np.uint8(np.minimum(np.maximum(int_pos, 0.0), 255.0))
    else:
        clk_neg[clk_y, clk_x] = 1
        int_neg = ndimage.distance_transform_edt(1 - clk_neg)
        int_neg = np.uint8(np.minimum(np.maximum(int_neg, 0.0), 255.0))
    ...
    output_image[np.where(output_image > 0.5)] = 1
    output_image[np.where(output_image <= 0.5)] = 0
    segmask = np.uint8(output_image[0, 0, :, :, 0])

    return segmask, int_pos, int_neg
